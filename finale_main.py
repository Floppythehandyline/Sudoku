#reusable sudoku module from main.py
...
def puzzle_to_array(p) -> np.ndarray:
    ...
    # Returns a numpy array of length 81 with integers 0-9 (0 for blanks).
    try:
        arr = np.array(p)
...
LEVELS = {
    # mapping level -> avg_rank used by dokusan
    # lower avg_rank = more cells filled (easier); higher avg_rank = fewer cells (harder)
...
def _generate_and_solve(level: str = "medium", avg_rank: Optional[int] = None) -> int:
    ...
    sod = Sudoku.from_dokusan(puzzle)
...
def select_level() -> str:
    """Display menu and prompt user to select difficulty level."""
    print("=" * 50)
